...
\begin{enumerate}
    ...
    \item Statistics
\end{enumerate}
The above systems are detailed in full in their respective subsections in Section 4: Sub-Systems of this report, but we will briefly outline the functionality of each here and discuss their high-level interaction with one another.

The Database back-end subsystem will serve as the data foundation for the application by providing data storage and look-up crucial for the functionality of all other subsystems. Each subsystem will interact with the database through an API developed for the application to facilitate passing of data back and forth.
...
The Geolocation module will provide the primary functionality of the application, which is the collection of geographic route data associated with a user account. This subsystem relies heavily on device sensors to generate accurate data as well as several web services to supplement the collected data.

The Geolocation and Statistics systems also employ a device side database for the storage of user specific Route and report information.
...
\begin{enumerate}
    \item Upon loading the application, the user is brought to the map screen. Functionality is disabled until a user has logged in.
    ...
    \subitem If the User is not currently near the beginning of their selected Route, they will be prompted to move to that location and to try recording again.
    \item From the map view, the user may use a navigation drawer panel to navigate to their profile, a summary of their social page, or metrics for the user and/or current route.
    \item From any of these views, the user may navigate to all other views using the navigation drawer.
    \item From the navigation view, user may select logout.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Link their Facebook account with the app to create an account
    \item Fill out a form based application to create an account
    \item The system will allow the user to challenge other users within their area through the application interface
    \item Compete for best time on routes in their area
    ...
    \item System will use device sensors to record user altitude.
    ...
    \item The user accounts system will display a user's Routes in a list view on the Routes page.
...
\begin{figure}
    \centering
    \begin{center}
        \makebox[\textwidth]{\includegraphics[width=\paperwidth]{infrastructure.png}}
    \end{center}
    \caption{Pathway Backend Sequence Diagram}
    \label{fig:my_label}
\end{figure}

\paragraph{Detailed Model - System Analysis}
...
\subsubsection{Introduction}
The Social subsystem will utilize some simplistic features that enable the user to interact with other users that are using the app within their local area. We will be implementing certain aspects from a Facebook API that will allow the user to integrate their Facebook profile when they first create an account. This will let a user expand their use of the app to include others. The user will also have a form based alternative for profile creation, which would give the user an opportunity to create a profile from the app. This would limit inclusiveness of other users, as social features would be disabled, and would only be used to keep records of that user's routes.
The app will give the users an option to choose a route in their specified location, which will then list the top times that the route was travelled. The user will be able to choose a top time that is displayed in the list and will be able to issue a "Challenge" , which is similar to a "poke" on Facebook. After the user picks the time they want to challenge on a certain route, and they complete it, the person that the user chose to challenge would be notified that their current record was broken.
We will also include an achievement system feature that will give incentives to the user based on certain accomplishments within the application.
...
\begin{figure}[H]
	\centering
	\begin{center}
		\makebox[\textwidth]{\includegraphics[width=0.4\paperwidth]{FBLoginScreenshot.png}}
	\end{center}
...
\begin{figure}[H]
	\centering
	\begin{center}
		\makebox[\textwidth]{\includegraphics[width=0.4\paperwidth]{AchievementsScreenshot.png}}
	\end{center}
...
\begin{figure}[H]
	\centering
	\begin{center}
		\makebox[\textwidth]{\includegraphics[width=0.4\paperwidth]{FriendsListScreenshot.png}}
	\end{center}
	\caption{User displaying the achievements page}
	\label{fig:my_label}
\end{figure}
...
//callback manager gets response code to determine next activity
...
\begin{lstlisting}
/*
code will access a local database that has the achievements
stored manually. Then we create an object of the class that
the local database is created in to pull user report data
so that we can set if statements which will check and see
if requirements are satisfied for the achievement
*/
...
/*
called after user chooses route to challenge and request
is made to server to retrieve phone number
*/
...
\paragraph{Design Alternative: Server-Side Implementation}
...
\paragraph{Second Design Alternative: Client-Side Implementation}
The second approach, which we decided to implement, was to implement the system on the android device. The system was implemented in very much the same manner as the original, however the reports were generated and stored on the android device's local SQLite database. This approach offered easier access to some of the resources the report system needed and better security for the User's route data. It also made it easier for the Statistics subsystem to be intergrated with the other subsystem. The reports could be immediately stored and retrieved from a local database implemented on the client device.
...
The above diagram shows the interaction between the classes detailed in Figure 18. Data sent to and retrieved from the database must move through the FetchData and SendData classes, which use asynchronous http url connections to send and received data.
In the event of data loss, the system is still able to save and retrieve the User's Routes from the local database instance.
...
The above activity diagrams shows the flow of user interaction with the system. The system is in an idle state until the user presses Start to begin recording a Route, new or existing.
From the map, a user can view info about a nearby Route.
After traversal, a user saves the Route by giving it a name and activity type, and optionally exports the Route data to the device. (Export is currently not implemented completely).
...
\newpage
\subsubsection{Design Analysis: Distance Algorithms}
...
\begin{itemize}
\item Simple End-to-End Comparison.
\item Hausdorff distance.
\item Fréchet distance.
\end{itemize}
The purpose of these algorithms is to determine which algorithm is most suitable for determining route similarity. These algorithms are crucial in determining whether or not a traversal of an existing Route by a user is similar enough to be considered the same Route.
\newline
\newline
Route similarity for our purposes was defined as such for two Routes, Route 1 and Route 2: Every vertices in Route 1 is within 25m of a vertices or the line between two vertices of Route 2.
Route 2 is by definition also similar to Route 1.
\newline
\newline
Input for all algorithms was two Route features and near tolerance was set at 25m. Algorithms were tested within a Java project in NetBeans, and then adapted to Android Studio as needed.

\pagebreak

\paragraph{Simple End-to-End Comparison}
...
The above implementation of the discrete Fréchet distance, as described by Eiter and Mannila\cite{g_frechetcompute_1994}, runs in O(p x q) for initial assignment and  O(p x q) for calculation of ca(p,q) for a total of O(p x q) time. This implementation makes use of dynamic programming so that it's run-time is no worse than the Hausdorff implementation detailed in Algorithm 2.
...
\subsubsection{Testing Plan and Overview}
...
\item Database - Refers to both the local device database and the external Pathway database.
\end {itemize}
\pagebreak
\subsubsection{Test Cases}
\begin {itemize}
...
\subitem Actual Result: Map view pans to current location, as shown in Figure 25 a and b.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.9\linewidth]{GeoTestInitialEmu.png}
    \caption{Start Location}
    \label{fig:Start Location}
    \end{subfigure}
    \begin{subfigure}{0.5\textwidth}
    \includegraphics[width=0.9\linewidth]{PathwayMainLayout.png}
    \caption{Current Location}
    \label{fig:Current Location}
    \end{subfigure}
    \caption{Initial Location and Location after button press.}
    \label{fig:my_label}
\end{figure}
...
\subitem Actual Result: Route is successfully drawn on screen. Figure 26 shows traversal in a localized area (roughly 100 ft).
\subitem Related Code:
...
\begin{figure}[H]
    \centering
    \begin{center}
        \makebox[\textwidth]{\includegraphics[totalheight=0.5\textheight]{GeoTestEmu2.png}}
    \end{center}
    \caption{Drawing of Route during traversal.}
    \label{fig:my_label}
\end{figure}
\pagebreak
...
\subitem Actual Result: Route continued to record while data was unavailable (Figures 27 a and b). Quality of data suffered, and was noisy when the User was stationary, due to resolution of GPS sensor, which produced false movement along the route. (Figure 28).
\subitem Related Code: No specific code was needed to ensure this functionality.
\begin{figure}[H]
    \begin{subfigure}{0.5\textwidth}
    \includegraphics[width=0.9\linewidth]{GeoTestNoDataDev1.png}
    \caption{After data loss.}
    \label{fig:Start Location}
    \end{subfigure}
    \begin{subfigure}{0.5\textwidth}
    \includegraphics[width=0.9\linewidth]{GeoTestNoDataDev2.png}
    \caption{Variance along back and forth route.}
    \label{fig:Current Location}
    \end{subfigure}
    \caption{Route Traversal after data loss.}
    \label{fig:my_label}
\end{figure}
\begin{figure}[H]
    \centering
    \begin{center}
        \makebox[\textwidth]{\includegraphics[totalheight=0.7\textheight]{GeoTestNoDataDev3.png}}
    \end{center}
...
\item TC-04: Save User Route
\subitem Requirement(s) Fulfilled:
\subsubitem System will store a user's routes in a particular format for use in Pathway.
\subitem Description: The device saves a User's Route to the database.
\subitem Precondition: User has traversed a Route.
\subitem Test Steps:
\subsubitem 1. User presses stop.
\subsubitem 2. User enters name and selects activity type.
\subsubitem 3. Route is saved to database.
\subsubitem 4. Message shows that Route is saved.
\subitem Expected Result: Message showing Route is has successfully saved is shown.
\subitem Actual Result: Route is saved and message is displayed. as shown in Figure 29.
\subitem Related Code:
\begin{lstlisting}[
basicstyle=\small
]
        ...
        public void onClick(View v) {
            ...
            String actType = dropdown.getSelectedItem()
            ...
            new SendData(getString(R.string.routesURL),
            currentRoute, MainActivity.this).execute();
...
\begin{figure}[H]
    \centering
    \begin{center}
        \makebox[\textwidth]{\includegraphics[totalheight=0.7\textheight]{GeoTestEmu4.png}}
    \end{center}
    \caption{Saving of Route.}
    \label{fig:my_label}
\end{figure}
\pagebreak
\item TC-05: Draw Nearby Routes
\subitem Requirement(s) Fulfilled:
\subsubitem System will display traversed routes in the User's immediate vicinity.
\subsubitem System will allow a user to tap on a route to retrieve and view information associated with that route.
\subitem Description: The device draws nearby Routes.
\subitem Precondition: User has logged in.
\subitem Test Steps:
\subsubitem 1. User changes location enough to trigger redraw of routes.
\subsubitem 2. Nearby Routes are drawn.
\subsubitem 3. Nearby Routes can be selected to show information.
\subitem Expected Result: Nearby Routes are shown after location change, as shown in Figure 30.
\subitem Actual Result: Nearby Routes are displayed.
\subitem Related Code:
\end{itemize}
\begin{lstlisting}[
basicstyle=\small
...
for (int i = 0; i < routesList.length(); i++) {
    try {
            ...
            Route tempRoute = new Route(tempJson.getString(
            "data").replace("activity", "atype")
            );
            PolylineOptions routeOptions = new PolylineOptions()
                    .color(Color.BLUE)
                    .width(16)
...
\begin{figure}[H]
    \centering
    \begin{center}
        \makebox[\textwidth]{\includegraphics[totalheight=0.7\textheight]{GeoTestEmu5.png}}
    \end{center}
    \caption{Nearby Routes drawn on the map, depicted by Blue lines.}
    \label{fig:my_label}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{REST API} - Representational State Transfer, provides interoperability between systems through the web.
...
\begin{itemize}
    ...
    \item - Docs.djangoproject.com. (2017). django.contrib.auth | Django documentation | Django. [online] Available at: https://docs.djangoproject.com/
    en/1.11/ref/contrib/auth/ [Accessed 17 Oct. 2017].
    ...
    \item - Developer.android.com. (2017). HttpURLConnection | Android Developers. [online] Available at: https://developer.android.com/reference/java/
    net/HttpURLConnection.html [Accessed 13 Nov. 2017].
